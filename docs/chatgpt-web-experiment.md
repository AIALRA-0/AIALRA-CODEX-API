...
新账号池默认均分流量，两个账号为 50% / 50%，三个账号为 34% / 33% / 33%。权重只在账号健康、已登录、已验证、空闲且不在冷却时生效；权重为 0% 的健康账号不接正常流量，只在没有正权重账号可用且消息尚未提交时作为备用；消息已经提交后不会因为账号故障而换号重发。首页显示的是浏览器本地读取并脱敏后的 Codex 订阅额度窗口，不把它冒充为网页聊天的无限额度，也不保存 Cookie、访问令牌、邮箱、上游账号标识或原始接口响应。
