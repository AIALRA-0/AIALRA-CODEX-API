...
Every durable queue item derives its active expiration from the task deadline and adds five minutes for result persistence and reset. Web generation time starts after an account lease is assigned, rather than while a task waits for capacity. Account leases are renewed while the owning Worker remains alive; a lost or expired lease quarantines the account instead of moving an uncertain task to another account. Web tasks never switch accounts after submission uncertainty.
