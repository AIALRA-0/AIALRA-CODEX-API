...
```bash
install -m 0644 deploy/systemd/aialra-model-router-runner-egress.service \
  /etc/systemd/system/aialra-model-router-runner-egress.service
systemctl daemon-reload
systemctl enable aialra-model-router-runner-egress.service
```
...
完成 4 个普通聊天、4 个搜索和 2 个深度研究真实任务后，只有成功数量至少 9/10、重复发送为 0、错误结果归属为 0 时才开放接单
